...
export class ExpenseService {
  ...
  async create(expense: CreateExpenseDTO): Promise<ReturnExpenseDTO[]> {
    ...
    const unique = await this.expenseRepository.isMonthlyUnique(desc,date[1],date[0]);
    
    if (!unique) throw new ConflictException('This expense already exists for this month');
  ...
  async update(id: number, newInfo: UpdateExpenseDTO): Promise<ReturnExpenseDTO> {
    ...
    const unique = await this.expenseRepository.isMonthlyUnique(desc,month,year,oldId);
  ...
  async delete(id: number): Promise<string> {
    ...
    if (deleteResult.affected == 0) return `Expense with id ${id} does not exist`;
